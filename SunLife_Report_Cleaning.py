...
# File Path
path = r"\\Innda11fs01v.mercer.com\scratch$\Canada Project\Files"
carrier = "Sunlife"
try:
    # Call function1()
    test_1 = report_1(path+"/report 1.xls")
    test_1.to_csv(f"{path}/{carrier}_Report_1_test.csv", index = False)
...
try:
    ...
    test_2.to_csv(f"{path}/{carrier}_Report_2_test.csv", index = False)
...
try:
    ...
    test_3.to_csv(f"{path}/{carrier}_Report_3_test.csv", encoding = "utf-8" ,index = False)
...
try:
    ...
    test_4.to_csv(f"{path}/{carrier}_Report_4_test.csv", encoding = "utf-8" ,index = False)
